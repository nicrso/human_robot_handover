...
class RGBDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return: 
        {
            ...
            'angle': np.ndarray (1,), np.float32, range [0, 180]
        }
        """
...
def get_center_angle(
    ...
    # Hint: it's a hack
    # ===============================================================================
    diff_arr = left_coord-right_coord
    center_coord, angle = (left_coord+right_coord)/2, (atan2(diff_arr[1], diff_arr[2])*180/np.pi) % 180
    print(center_coord)
    # ===============================================================================
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        cntr = data_torch['center_point']
        ang = data_torch['angle'].numpy()
        
        #get finger points 
        left_pt, right_pt = get_finger_points(cntr, ang)
        
        img_shape = img.size()
        
        kps = KeypointsOnImage([
            Keypoint(x=left_pt[0], y=left_pt[1]),
            Keypoint(x=right_pt[0], y=right_pt[1])
        ], shape=img_shape[:2])
        
        img_aug, kps_aug = self.aug_pipeline(image=img, keypoints=kps)
        
        center_point, angle = get_center_angle(kps_aug.keypoints[0], kps_aug.keypoints[1])
        
        data_torch['rgb'] = (torch.from_numpy(img_aug)).to(torch.uint8)
        data_torch['center_point'] = center_point
        data_torch['angle'] = angle
