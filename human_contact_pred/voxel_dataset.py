...
# Train Set (['camera', 'toothbrush', 'water_bottle', 'binoculars', 'flute', 'light_bulb', 'hammer', 'bowl', 'ps_controller', 'utah_teapot', 'flashlight', 'eyeglasses', 'hand', 'cup', 'cell_phone', 'stapler', 'apple', 'knife', 'mouse', 'door_knob', 'banana', 'toothpaste'])
# Val Set (['pan', 'mug', 'scissors', 'wine_glass', 'headphones'])
...
class VoxelDataset(tdata.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if abs(self.random_rotation) > 0:
      ai = np.random.uniform(-np.pi*self.random_rotation/180,
        np.pi*self.random_rotation/180)
      aj = np.random.uniform(-np.pi*self.random_rotation/180,
        np.pi*self.random_rotation/180)
      ak = np.random.uniform(-np.pi*self.random_rotation/180,
        np.pi*self.random_rotation/180)
      
      R = txe.euler2mat(ai, aj, ak)
